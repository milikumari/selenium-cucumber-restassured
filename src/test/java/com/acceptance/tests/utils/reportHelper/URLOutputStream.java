...
class URLOutputStream extends OutputStream {
    ...
    private com.acceptance.tests.utils.reportHelper.URLOutputStream.ResponseException createResponseException(int responseCode, IOException expected, InputStream errorStream)
            throws IOException {
        try (BufferedReader br = new BufferedReader(new InputStreamReader(errorStream, UTF_8))) {
            ...
            return new com.acceptance.tests.utils.reportHelper.URLOutputStream.ResponseException(responseBody, expected, responseCode, contentType);
        }
    }
